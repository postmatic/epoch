...
jQuery( document ).ready( function ( $ ) {
    ...
    (function ( $, app ) {
        ...
        app.init = function() {
			...
			$( app.comments_wrap_el ).replaceWith( $( '<iframe>', { 'id' : app.comments_wrap_el.id, 'style' : 'width:100%' } ) );
			app.comments_wrap_el = document.getElementById( epoch_vars.comments_wrap );
            app.iframe = (app.comments_wrap_el.contentWindow) ? app.comments_wrap_el.contentWindow : (app.comments_wrap_el.contentDocument.document) ? app.comments_wrap_el.contentDocument.document : app.comments_wrap_el.contentDocument;
            

			var stylesheet = '<script type="text/javascript" src="http://build.tricodia.com/wp-includes/js/comment-reply.min.js"></script><script>var stylesheet = document.createElement("link");stylesheet.type = "text/css";stylesheet.rel = "stylesheet";stylesheet.href = "' + epoch_vars.stylesheet + '"; document.head.appendChild( stylesheet );</script>';
			app.iframe.document.write( stylesheet + app.form_wrap_el.innerHTML );
			console.log( app.form_wrap_el );
            app.comments_wrap_el = document.getElementById( epoch_vars.comments_wrap );
        ...
        app.put_comment_in_dom = function( html, parent_id, level, id ) {
            ...
            if ( 0 == comment.comment_parent && 'DESC' == epoch_vars.epoch_options.order ) {
                ...
                if ( 0 == parent_id ) {
                    ...
                    app.iframe.document.write( html );
                    $( app.comments_wrap_el ).height( $( app.iframe ).outerHeight() );
