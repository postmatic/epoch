...
null!=a.form_el&&(a.form_el.removeAttribute("action"),a.form_el.setAttribute("action","post"));a.set_width();a.comments_open();a.comment_count(!1);window.onresize=function(c){a.set_width()};epoch_vars.live_mode&&Visibility.every(epoch_vars.epoch_options.interval,function(){0==a.shut_it_off&&a.comment_count(!0)});b(document).on("submit","#"+epoch_vars.form_id,function(c){c.preventDefault();a.shut_it_off=!0;var f=!1,d=[];b(a.form_el).find("select, textarea, input").each(function(){b(this).prop("required")&&
...
a()})}))})};a.comments_open=function(){a.shut_it_off=!0;b.post(epoch_vars.api_url,{action:"comments_open",epochNonce:epoch_vars.nonce,postID:epoch_vars.post_id}).fail(function(c){a.shut_it_off=!1}).success(function(c){c=a.get_data_from_response(c);1==c?a.shut_it_off=!1:a.comments_closed=!0})};a.comment_count=function(c){null!=epoch_vars.alt_comment_count?a.check_comment_count_from_file(c):a.comment_count_from_wp(c)};a.comment_count_from_wp=function(c){a.shut_it_off=!0;b.post(epoch_vars.api_url,{action:"comment_count",
epochNonce:epoch_vars.nonce,postID:epoch_vars.post_id}).fail(function(c){a.shut_it_off=!1}).success(function(b){b=a.get_data_from_response(b);"undefined"!=b.count&&0<b.count?(c?b.count>a.last_count&&a.new_comments():a.get_comments(),a.set_last_count(b.count)):a.no_comments=!0;a.shut_it_off=!1})};a.check_comment_count_from_file=function(c){a.shut_it_off=!0;b.get(epoch_vars.alt_comment_count,{}).fail(function(){a.comment_count_from_wp(c)}).success(function(b){"undefined"!=b&&0<b?(c?b>a.last_count&&
a.new_comments():a.get_comments(),a.set_last_count(b)):a.no_comments=!0;a.shut_it_off=!1})};a.get_comments=function(){a.shut_it_off=!0;b.post(epoch_vars.api_url,{action:"get_comments",epochNonce:epoch_vars.nonce,postID:epoch_vars.post_id,highest:0}).done(function(c){a.shut_it_off=!1}).success(function(c){c=a.get_data_from_response(c);a.comment_response(c,!1);jQuery("body").triggerHandler("epoch.comments.loaded")})};a.comment_response=function(c,d){if("object"==typeof c&&"undefined"!=c&&"undefined"!=
c.comments&&(comments=c.comments,comments=JSON.parse(comments),"undefined"!==comments&&0<comments.length)){var e=[],g=[];b.each(comments,function(c,b){id=parseInt(b.comment_ID,10);a.highest_id<id&&(a.highest_id=id);0==b.comment_parent?e.push(b):g.push(b)});"ASC"==epoch_vars.epoch_options.order?(g.reverse(),e.reverse()):g.reverse();b.each(e,function(c,e){html=a.parse_comment(e);a.put_comment_in_dom(html,e.comment_parent,e.depth,parseInt(e.comment_ID,10));d&&(comment_el=document.getElementById("comment-"+
e.comment_ID),null!=comment_el&&b(comment_el).addClass("epoch-success").delay(100).queue(function(a){b(this).removeClass("epoch-success");a()}))});b.each(g,function(c,b){html=a.parse_comment(b);a.put_comment_in_dom(html,b.comment_parent,b.depth,parseInt(b.comment_ID,10))})}};a.new_comments=function(){a.shut_it_off=!0;b.post(epoch_vars.api_url,{action:"new_comments",epochNonce:epoch_vars.nonce,postID:epoch_vars.post_id,highest:a.highest_id}).done(function(c){a.shut_it_off=!1}).success(function(c){c=
a.get_data_from_response(c);a.comment_response(c,!0)})};a.get_data_from_response=function(a){return a.data};a.parse_comment=function(c){parent_id=c.comment_parent;source=b(a.template_el).html();template=Handlebars.compile(source);return html=template(c)};a.put_comment_in_dom=function(c,d,e,g){e>a.max_depth&&(alert(),e=a.max_depth);0==d&&"DESC"==epoch_vars.epoch_options.order?(first_child=a.comments_wrap_el.firstChild,new_el=document.createElement("div"),new_el.innerHTML=c,a.comments_wrap_el.insertBefore(new_el,
first_child)):0==d?b(c).appendTo(a.comments_wrap_el):(c='<div class="epoch-child child-of-'+d+" level-"+e+' ">'+c+"</div>",parent_el=document.getElementById("comment-"+d),null!=parent_el?b(c).appendTo(parent_el):b(c).appendTo(a.comments_wrap_el));b(".comment-reply-link").click(function(a){a.preventDefault()})};a.set_width=function(){el=document.getElementById(epoch_vars.sniffer);null!=el&&(content_width=b(el).parent().outerWidth(),"number"==typeof content_width&&(wrap_el=document.getElementById(epoch_vars.wrap_id),
b(wrap_el).css("width",content_width)))};a.set_last_count=function(c){a.last_count=c;b(a.count_el).text(c)}})(jQuery,window.Epoch||(window.Epoch={}))});jQuery(function(){Epoch.init()});